...
public class SecretarioDAOPostgresImpl implements SecretarioDAO {
    ...
    @Override
    public int insert(Secretario secretario) {
        ...
        String querySecretario = String.format("INSERT INTO %s (%s, %s) VALUES (?, ?)",
                SchemDB.TAB_SECRETARIO, SchemDB.COL_USUARIO_ID, SchemDB.COL_SECRETARIO_TURNO);
        ...
        try {
            ...
            preparedStatement.setString(1, secretario.getEstado().name());
            preparedStatement.setString(2, secretario.getNombre());
            preparedStatement.setString(3, secretario.getApellido());
            preparedStatement.setString(4, secretario.getEmail());
            preparedStatement.setString(5, secretario.getPassword());
            preparedStatement.setString(6, secretario.getTelefono());
            ...
            // Si el usuario se insertó bien y tenemos el ID, insertamos el Secretario
            if (filasUsuario > 0 && idGenerado != -1) {

                // Actualizamos el objeto Java con su nuevo ID
                secretario.setIdUsuario(idGenerado);

                // Preparamos el insert del Secretario usando el ID que acabamos de obtener
                preparedStatement = connection.prepareStatement(querySecretario);
                preparedStatement.setInt(1, idGenerado);
                preparedStatement.setString(2, secretario.getTurno()); // Atributo exclusivo de Secretario
            ...
            System.out.println("❌ ERROR (insert secretario): " + e.getMessage());
            try {
    ...
    @Override
    public List<Secretario> selectAll() {
        List<Secretario> listaSecretarios = new ArrayList<>();

         /* INNER JOIN
         En lugar de hacer una consulta a 'secretario' y luego lanzar un 'SELECT * FROM usuario'
         dentro de un bucle por cada secretario que encontremos (Nivel 1, muy lento y poco eficiente),
        ...
        String query = String.format("SELECT * FROM %s u INNER JOIN %s sec ON u.%s = sec.%s",
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                EstadoUsuario estado = EstadoUsuario.valueOf(resultSet.getString(SchemDB.COL_USUARIO_ESTADO).trim().toUpperCase()); // Parseo directamente aquí , me resulta más fácil de ver que luego al crear secretario y añadirlo a lista.
                ...
                // Recogemos el atributo específico del hijo (tabla secretario)
                String turno = resultSet.getString(SchemDB.COL_SECRETARIO_TURNO);

                // Ensamblamos el objeto usando el constructor manual con super() que creamos en la clase Secretario.
                // Este constructor se encarga de repartir los datos internamente entre la clase Padre e Hija.
                listaSecretarios.add(new Secretario(id, estado, nombre, apellido, email, password, telefono, turno));
            }
        } catch (SQLException e) {
            System.out.println("❌ ERROR (selectAll secretarios): " + e.getMessage());
        }
    ...
    public int update(Secretario secretario) {
        ...
        String querySecretario = String.format("UPDATE %s SET %s=? WHERE %s=?",
                SchemDB.TAB_SECRETARIO, SchemDB.COL_SECRETARIO_TURNO, SchemDB.COL_USUARIO_ID);

        try {
            // INICIO TRANSACCIÓN
            connection.setAutoCommit(false);

            //Cargamos y ejecutamos la actualización del Padre
            ...
            preparedStatement.setString(5, secretario.getPassword());
            preparedStatement.setString(6, secretario.getTelefono());
            preparedStatement.setInt(7, secretario.getIdUsuario()); // El WHERE para saber a quién actualizamos
            preparedStatement.executeUpdate();

            // Cargamos y ejecutamos la actualización del Hijo
            preparedStatement = connection.prepareStatement(querySecretario);
            preparedStatement.setString(1, secretario.getTurno());
            preparedStatement.setInt(2, secretario.getIdUsuario()); // El mismo WHERE
            preparedStatement.executeUpdate();
            ...
            System.out.println("❌ ERROR (update secretario): " + e.getMessage());
            try {
                // Si el Update de Secretario falla, deshacemos el Update de Usuario
                if (connection != null) connection.rollback();
            } catch (SQLException ex) { }
        } finally {
            try {
    ...
    public int delete(int id) {
        // EL ORDEN ES VITAL EN BASES DE DATOS RELACIONALES:
        ...
        //Solución: Primero borramos al hijo dependiente (Secretario), y cuando el padre quede liberado, lo borramos a él (Usuario).

        String querySecretario = String.format("DELETE FROM %s WHERE %s=?", SchemDB.TAB_SECRETARIO, SchemDB.COL_USUARIO_ID);
        ...
        try {
            ...
            // Borramos primero al hijo (Secretario)
            preparedStatement = connection.prepareStatement(querySecretario);
            ...
            System.out.println("❌ ERROR (delete secretario): " + e.getMessage());
            try {
    ...
    /*private EstadoUsuario parseEstado(String estadoStr) {
        if (estadoStr == null) return EstadoUsuario.INACTIVO; // Lo de Inactivo era para inicializarlo en inactivo , si no tena estado lo dejo por curiosidad, pero no me convence.
        return EstadoUsuario.valueOf(estadoStr.trim().toUpperCase());
    }*/
}